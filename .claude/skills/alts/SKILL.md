---
name: alts
description: 用于维护 Atlas/alts 仓库内的产品规格和 Intake 流程;当任务涉及录入新产品、更新 STATUS.md、新增或补全功能点、维护 meta.yml、新增或更新 contract、执行 Intake Discover/Interview/Finalize、处理对象级反馈池或全局需求池、生成/修订 CONVENTIONS.md 时使用。
---

<!-- MIRROR of AGENTS.md (canonical). 两份文件正文必须语义等价;若不一致,以 AGENTS.md 为准。修改任意一份时同步另一份。-->
<!-- 本 skill 用于 Atlas 内部 Agent 操作(读写 data/products/<id>/ 下的 md 文件),
     与对外的 revise/generate prompt 模板不冲突——后者是 Atlas UI 通过端点为外部 Agent
     提供的提示词,本 skill 是 Claude Code 直接在仓库内操作时遵循的规则。 -->

# alts skill

Atlas/alts is an agent-native, local-first product specification system. This skill applies when maintaining Atlas product specs, Intake outputs, `STATUS.md`, `meta.yml`, feature rows, or cross-product contracts. It does not apply to general application coding.
