...
- **并行友好(重要)**:本改动可能与其它功能点**并行建造**。把代码尽量**自包含在自己模块目录**(如 src/modules/<module>/...),通过既有模块/路由**注册表**接入。**尽量不改 src/App.tsx、全局样式等共享入口文件**(并行时改同一文件会合并冲突);确需改共享文件时,**只追加**自己的注册项,**不要改动或删除已有内容/别的模块的代码**。`
  ].join("\n");
...
export async function buildScaffoldInstruction(productId: string): Promise<string> {
  ...
  return [
    ...
    "",
    `**并行友好架构(重要)**:后续会有多个功能点**并行**往里填代码。请把入口做成**数据驱动 / 自动发现**:`,
    `- 入口(如 App.tsx)**遍历模块/功能注册表渲染**,不要为单个功能点写死组件;新功能点只需在自己模块目录里登记一项,**无需改动入口文件**。`,
    `- 提供清晰的「每个功能点该怎么自包含地接入」约定(写进 README),让后续建造**只动自己模块目录**、避免改全局共享文件 → 避免并行合并冲突。`,
    "",
    `不要实现具体业务逻辑(那是后续逐模块建造的事);只要地基稳、能跑、结构清楚、并行友好。最小依赖,别堆无关库。`
  ].join("\n");
}
